...
ip_add=`ip addr show | grep "inet " | grep -v "127.0.0.1" | awk '{print $2}' | cut -d '/' -f 1`
num_proc=`ps -ef | wc -l`
root_fs_pc=`df -h / | tail -1 | awk '{print$5}'`
root_fs_pc_numeric=`df -h / | tail -1 | awk '{print$5}' | cut -f1 -d'%'`
...
html="/tmp/Server-Health-Report-`hostname`-`date +%y%m%d`-`date +%H%M`.html"
